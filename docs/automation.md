...
3. 分割直後に整合性を assert する. 全チャンクを連結したら原文と一致する
   ことをスクリプトで検査する (`連結 == 原文`). 部分範囲指定の場合は
   原文の該当範囲 (経題 + 指定段落) との一致を検査する
...
- 見出し "## <段落番号>" はチャンク先頭の段落番号から取り, 番号のない
  ...
  本文中の最後の番号を引き継ぐ. 先頭チャンクに番号がない場合 (Dhp-a の
  vatthu は物語導入部のあと偈の位置に初めて番号が現れる. Khemātherīvatthu
...
- 番号規則で決まらない見出しは assemble_md.rb の第 5 引数 (taiyaku.sh の
  ...
  上書きは抽出段階でなく組み立て段階に置く. verify_taiyaku.rb は
  範囲形式 "## <n>-<m>" を許可し, 欠番検査では範囲を展開して数える
...
- 引用符とアポストロフィの規則 (2026/07/24 に生成プロンプトへ明文化):
  語句の引用は半角の ", アポストロフィは半角の ' を使い, “ ” や
  ‘‘ ’’ を新たに作らない. 再掲行は原文の表記のまま写す (原文の ‘‘ ’’ や
  綴りもそのまま). VRI 正本にアポストロフィはなく (省音は続け書き),
  対訳の ' は語の切れ目を示す編集表記
...
2. 生成後の diff チェック. 対訳の各行はパーリ語を再掲するため, LLM 生成で
   ...
   - 実装 (scripts/verify_taiyaku.rb) は番号付き行を対象に, 正本の連続
     ...
     - OK-NOSPACE: さらに引用符 (‘ ’ と " ') と空白をすべて除いて一致.
       引用符の省略・変形, `…pe…` 前後の空白付加, 連声の分かち書き
       (pajānātī’’ti -> pajānātī ti) など再掲時の編集的な揺れ.
       引用符はプロンプトで表記を規定していないため出力に揺れが出る.
       ASCII の " と ' はユーザのプリファレンスにより許容し (2026/07/24),
       それ以外 (全角の “ ” など) は NG で検出して " へ修正する
       (対訳では VRI の ‘‘ ’’ も用いないプリファレンスのため,
       修正先は ‘‘ ’’ ではなく " とする).
