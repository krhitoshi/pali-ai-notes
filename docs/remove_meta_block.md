...
(モデル) と生成日を記したメタブロックが付いている. 見出しは生成元の名前
(Claude / Gemini など) で, 完成後は不要なので削除する.
...
# 見出しは生成元名 (Claude / Gemini など ASCII). 日本語の内容見出しは除外される
...
1. 適用前に, モデル行の直後 (空行を挟んだ次の行) が必ずしも `### 対訳` でない
   ...
   これらは巻き込まない
2. 削除後に残存ゼロを確認: `grep -cE '^### (Claude|Gemini)'` と モデル文字列の grep
3. **git diff で削除内容を確認**: 挿入 0, 削除は `### Claude` / 日付 / モデル / 空行
